...
class CatalogController extends Controller {
    ...
    public function getShow($id) {
        $movie = Movie::find($id);
        return view('catalog.show', array('id' => $movie));
    }
    ...
    public function putEdit(Request $request, $id){
        ...
        return redirect('catalog/show/' . $m2->id);
    }
    
    public function putRent(Request $request, $id){
        $m1 = Movie::findOrFail($id);
            $m1->rented = $request->input('rented', true);
            $m1->save();
            Notification::success('Has alquilado la película correctamente.');
            return redirect('catalog/show/' . $m1->id);            
    }
    ...
    public function putReturn(Request $request, $id){
        $m1 = Movie::findOrFail($id);
            $m1->rented = $request->input('rented', false);
            $m1->save();
            Notification::success('Has devuelto la película correctamente.');
            return redirect('catalog/show/' . $m1->id);   
    }
